...
- **`supabase/enrich-places.mjs`** — Node script (ESM) that fills in the detail-page columns: for every place with `enriched_at is null`, it searches the Google Places API (New) for a match, uploads its photos to the `place-photos` Storage bucket, asks Claude to extract commonly-mentioned dishes from the reviews, and writes it all back to that place's row. Also backfills the list-card `image` column from the first uploaded photo, but only if `image` isn't already set (so a hand-curated `data.json` image is never overwritten on a re-enrich). Run via `npm run enrich` inside `supabase/`; safe to re-run since already-enriched rows are skipped.
